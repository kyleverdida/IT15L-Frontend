    ...
    <>
      {open ? (
        <div className="fixed bottom-6 right-6 w-96 max-w-[calc(100vw-3rem)] bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-50 chatbot-popup">
              ...
              >
                <div
                  className={`max-w-[85%] px-4 py-2 rounded-2xl text-sm
                    ${m.role === 'user' ? 'bg-cyan-500 text-white rounded-br-md' : 'bg-slate-100 text-slate-700 rounded-bl-md'}`}
                >
                  {m.text}
                </div>
              </div>
          ...
          <form onSubmit={sendMessage} className="p-3 border-t flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/50"
            />
            <button
              type="submit"
              className="p-2 bg-cyan-500 text-white rounded-xl hover:bg-cyan-600 transition"
            >
              <Send size={18} />
            </button>
          </form>
        ...
        <button
          ...
          className="fixed bottom-6 right-6 p-4 bg-gradient-to-r from-cyan-500 to-indigo-600 text-white rounded-2xl shadow-lg hover:shadow-xl transition z-50"
        >
